...
class  ML_Algoritms:
                ...
                for alName in self.alNames:
                    ...
                    for fold_number in range(1, number_of_splits+1):  
                        ...
                        augmented_columns = [
                            ...
                            if col.startswith('fp_') or col.startswith('rule_')
                        ]
                        ...
                        if alName != "HGB":
                            ...
                            self.algorithms[alName]["param_grid"]['class_weight'] = [class_weight_dict]
                        ...
                        if group_column != None:
                            wantedGroups = df_train[group_column]
                    ...
                    if (
                        ...
                        self.plot_algorithmResults(alName, all_y_test, all_y_proba, result_path)
    ...
    def calc_results(
        ...
        for i in range(len(all_y_test)):
            ...
            # ROC-AUC Score
            if len(np.unique(y_test)) > 1:  # Ensure y_test has more than one class
                fpr, tpr, _ = roc_curve(y_test, y_proba[:, 1])
                auc_roc = auc(fpr, tpr)
            else:
                auc_roc = np.nan
            ...
            if len(np.unique(y_test)) > 1:  # Ensure y_test has more than one class
                ...
                specificity = np.round(tn/(fp+tn), 4) if (fp+tn) > 0 else np.nan
                sensitivity = np.round(tp/(tp+fn), 4) if (tp+fn) > 0 else np.nan
            else:
                tn, fp, fn, tp = (0, 0, 0, 0)
                specificity = np.nan
                sensitivity = np.nan
